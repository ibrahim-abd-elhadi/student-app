...
try {
  ...
  const socket = initSocket(base);

  socket.emit("student:join", {
    deviceName: navigator.userAgent,
    studentId: data.user.id,
    name: data.user.display_name,
    classroomId: data.user.classroom_id,
    status: "online"
  });
...
<form className="card" style={{ width: 360 }} onSubmit={submit}> <h2>تسجيل دخول الطالب</h2>
...
function HostStatus() {
return ( <div className="center"> <div className="muted">
جلسة الطالب نشطة في الخلفية. تحت تصرف المعلّم. </div> </div>
);
}
